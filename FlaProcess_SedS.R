...
#elapsed time
row <- gsub(pattern="([Guaymas|Med|Marmara]+)-([A-Za-z0-9]+)-([0-9]+)cm-([a-zA-Z0-9]+)-([a-z]+)-t([0-9])",replacement="\\1-\\2-\\3cm-\\4-\\5-1-t\\6",x=row.names(master))
...
maxes <- matrix(ncol=ncol(factors),dimnames=list(NULL,colnames(factors)))
...
for (slice in levels(factors$core.depth)) {
    ...
    for (sub in levels(factors$substrate)) {
        #and one treatment...
        for (treat in levels(factors$treatment)) {
...
#remove row with NA
maxes <- maxes[complete.cases(maxes)==TRUE,]
