...
prex1TrtColors <- c(colors[3],brewer.pal("RdYlGn",n=3))
prex2TrtColors <- c(colors[3],brewer.pal("OrRd",n=3))
prex3TrtColors <- c(colors[3],"grey93",colors[4])
...
print("Result of paired t-test, change in integrated fluorescence intensity from notrt to trt (in raw FUs), Fig 2a:")
print(intensities_test)
print(paste("median change in fluorescence intensity (FUs):",median(intensities_complete$Diff)))
print("Result of t-test, percent improvement in fluorescence intensity from notrt to trt, Fig 2b:")
...
png("figures/Fig2a_PercentImprovementFluorescence.png",width=85,height=90,units="mm",res=900)
...
##Fig 2b
trtsFl <- intensities_melt[intensities_melt$variable=="trt"|intensities_melt$variable=="notrt",]
png("figures/Fig2b_ChangeFUNotrtTrt.png",width=85,height=90,units="mm",res=900)
ggplot(trtsFl,aes(x=variable,y=value)) + geom_boxplot(fill=TreatColors) + geom_line(color="grey25",alpha=0.1,size=0.25,aes(group=Inc)) + coord_cartesian(ylim=c(0,300000000)) + theme_bw() + labs(x="",y="fluorescence intensity (FU)") + scale_x_discrete(labels=c("no treatment","competitive desorption + SDS")) + theme(axis.title.y=element_text(vjust=1),axis.title.x=element_text(vjust=-0.4),axis.text=element_text(size=7))
dev.off()
...
### Fig 3
png("figures/Fig3_PercentImprovementHMWvsLMW.png",width=85,height=75,units="mm",res=900)
...
##Fig 4
png("figures/Fig4_DecreaseMaxHydrolRateNotrtTrt.png",width=85,height=80,units="mm",res=900)
...
## Fig 5a - horizontal barplot Guaymas 
png("figures/Fig5a_GuaymasMaxRatesBarplot.png",width=180,height=50,units="mm",res=900)
...
## Fig 5b - barplot Med
png("figures/Fig5b_MedMaxRatesBarplot.png",width=85,height=150,units="mm",res=900)
...
###################### Figure 6 ########################
#uses percentFl defined in fig 2a
theme_fig6 <- theme_bw() + theme(strip.background=element_rect(fill="grey93"),axis.text.y=element_text(size=6),axis.text.x=element_blank(),axis.title=element_text(size=7),strip.text=element_text(size=5,face="bold"),legend.position="none")
png("figures/Fig6_PercentImprovementByLocation.png",width=180,height=48,units="mm",res=900)
ggplot(percentFl,aes(x=variable,y=value)) + geom_boxplot(width=0.5,size=0.3,outlier.size=1,aes(fill=location)) + theme_fig6 + labs(x="Site (Core.Depth)",y="Improvement in Fluorescence Intensity (%)") + coord_cartesian(ylim=c(0,750)) + scale_y_continuous(breaks=c(0,100,300,500,700)) + facet_grid(.~core.depth) + scale_fill_manual(values=LocationColors)
dev.off()
...
###################### Figure 7 ########################
#uses percentFl from fig 2a
theme_fig7 <- theme_bw() + theme(axis.title=element_text(size=9,vjust=1),axis.text=element_text(size=7),legend.position="none")
png("figures/Fig7_PercentImprovementBySubstrate.png",width=85,height=75,units="mm",res=900)
ggplot(percentFl,aes(x=substrate,y=value)) + geom_boxplot(aes(fill=substrate)) + theme_fig7 + labs(x="",y="Improvement in Fluorescence Intensity (%)") + coord_cartesian(ylim=c(-10,500)) + scale_fill_manual(values=SubstrateColors)
dev.off()
...
prex2_melt$variable <- factor(prex2_melt$variable,levels=c("notrt","X700sds","X1400sds","X2800sds"))
...
theme_fig8 <- theme_bw() + theme(legend.position="none",axis.title=element_text(size=8,vjust=1),axis.text=element_text(size=5))
#Fig 8a, prex1
png("figures/Fig8a_PreX1Fluor.png",width=85,height=75,units="mm",res=900)
ggplot(prex1_melt,aes(x=variable,y=value)) + geom_boxplot(width=0.8,size=0.3,outlier.size=1,aes(fill=variable)) + scale_fill_manual(values=prex1TrtColors) + scale_x_discrete(labels=c("no treatment","competitive desorption","pH10","pH11")) + theme_fig8 + labs(x="",y="Integrated Fluorescence (FUs)")
dev.off()

#Fig 8b, prex2
png("figures/Fig8b_PreX2Fluor.png",width=85,height=75,units="mm",res=900)
ggplot(prex2_melt,aes(x=variable,y=value)) + geom_boxplot(width=0.8,size=0.3,outlier.size=1,aes(fill=variable)) + scale_fill_manual(values=prex2TrtColors) + scale_x_discrete(labels=c("no treatment","700uM","1400uM","2800uM")) + theme_fig8 + labs(x="",y="Integrated Fluorescence (FUs)") + coord_cartesian(ylim=c(400000,1100000))
dev.off()

#Fig 8c, prex3
png("figures/Fig8c_PreX3Fluor.png",width=85,height=75,units="mm",res=900)
ggplot(prex3_melt,aes(x=variable,y=value)) + geom_boxplot(width=0.8,size=0.3,outlier.size=1,aes(fill=variable)) + scale_fill_manual(values=prex3TrtColors) + scale_x_discrete(labels=c("no treatment","2800 -SDS","2800 +SDS")) + theme_fig8 + labs(x="",y="Integrated Fluorescence (FUs)")
dev.off()

prex1_test <- t.test((((prex1$CD-prex1$notrt)/prex1$notrt)*100),alternative="greater")
print("average % increase in peak fluorescence using competitive desorption in PreX1:")
print(paste(round(as.numeric(prex1_test$estimate),1),"; P=",round(prex1_test$p.value,3)))
...
print(paste(round(as.numeric(prex2_test$estimate),1),"; P=",round(prex2_test$p.value,3)))

prex3_test <- t.test((((prex3$sds-prex3$notrt)/prex3$notrt)*100),alternative="greater")
print("average % increase in peak fluorescence units using SDS in PreX3:")
print(paste(round(as.numeric(prex3_test$estimate),1),"; P=",round(prex3_test$p.value,3)))


############# Figure 9  ###############
#example of treatment improvement on chromatogram quality
