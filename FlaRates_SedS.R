...
for (j in 1:length(RatesMasterList)) {
    ...
    for (x in 1:length(TimeList[[j]])) {
        ...
        names(R2) <- gsub(pattern="([Guaymas|Med]+)-([A-Z0-9]+)-([0-9]+)cm-(trt|notrt)-([a-z]+)-([0-9a-z]+)-t([0-9])","rate.\\6.nM.hr",names(R2))
        names(kcR2) <- gsub(pattern="([Guaymas|Med]+)-([A-Z0-9]+)-([0-9]+)cm-(trt|notrt)-([a-z]+)-([0-9a-z]+)-t([0-9])","kcrate.\\6.nM.hr",names(kcR2))
